...
describe("organizationJsonLd", () => {
  it("returns valid Organization schema", () => {
    const result = organizationJsonLd("SwiftCart", "https://swiftcart.ch");
    expect(result["@context"]).toBe("https://schema.org");
    expect(result["@type"]).toBe("Organization");
    expect(result.name).toBe("SwiftCart");
    expect(result.url).toBe("https://swiftcart.ch");
  });
});
...
describe("productJsonLd", () => {
  ...
  it("returns valid Product schema with variants", () => {
    ...
    expect(result.url).toBe("https://swiftcart.ch/de/products/t-shirt");
  ...
  it("uses basePrice when no variants", () => {
    const result = productJsonLd(baseProduct, [], [], "https://swiftcart.ch", "en");
    expect(result.offers.lowPrice).toBe("29.90");
  ...
  it("shows InStock when total stock > 0", () => {
    ...
    const result = productJsonLd(baseProduct, variants, [], "https://swiftcart.ch", "de");
  ...
  it("defaults to InStock when no variants (stock unknown)", () => {
    const result = productJsonLd(baseProduct, [], [], "https://swiftcart.ch", "de");
    expect(result.offers.availability).toBe("https://schema.org/InStock");
  });

  it("handles null description", () => {
    const product = { ...baseProduct, description: null };
    const result = productJsonLd(product, [], [], "https://swiftcart.ch", "de");
    expect(result.description).toBeUndefined();
  });

  it("includes multiple images", () => {
    const images = [{ url: "https://img.test/1.jpg" }, { url: "https://img.test/2.jpg" }];
    const result = productJsonLd(baseProduct, [], images, "https://swiftcart.ch", "de");
...
describe("breadcrumbJsonLd", () => {
  it("returns valid BreadcrumbList schema", () => {
    const items = [
      { name: "Home", url: "https://swiftcart.ch/de" },
      { name: "Products", url: "https://swiftcart.ch/de/products" },
    ];
